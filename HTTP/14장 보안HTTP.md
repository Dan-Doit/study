...
- 만약 URL이 https 스키마를 가지고있다면, 클라이언트는 서버에 보안기본포트 `(:443)` 로 연결하고 서버와 바이너리포맷으로된 몇몇 SSL보안 매개변수를 교환하면서 암호화된 HTTP 명령이 뒤를 잇는다.
...
암호화되지 않은 HTTP에서, 쿨라이언트는 웹 서버의 80번 포트로 TCP 커넥션을 열고, 요청/응답 메시지를 받고 커넥션을 닫는다.
...
SSL이 어떨게 사용되는가에 따라 이는 더 복잡해 질수있지만 일반적인 개념을 보여주고있다.



### 서버 인증서

오늘날 클라이언트 인정서는 웹 브라우징에선 흔히쓰이지 않는다.

대부분의 사용자는 개인 클라이언트 인정서를 갖고 있지도 않다.
